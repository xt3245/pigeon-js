...
describe('build', function() {
  ...
  it('should support nested objects', function(done) {
    ...
    pigeon.render(data, function(err, html) {
      ...
      expect(html)
      .to.equal(
        '<body><div class="class"><div class="child"><div class="finalchild">Content</div></div></div></body>'
      );
      done();
    });
  ...
  it('should support same keys', function(done) {
    ...
    pigeon.render(data, function(err, html) {
      ...
      expect(html)
      .to.equal(
        '<div class="class"><div class="child">Content</div><div class="child">Content</div><div class="child">Content</div></div>'
      );
      done();
    });
...
describe('parse', function() {
  ...
  it('should parse classes', function() {
    ...
    var string =
    '.classes.anotherclass.lastclass.anotherclass.classes._545^&3fg4',
     parsed = parse(string);
     
     expect(parsed.attributes)
     .to.exist;
     expect(parsed.attributes)
     .to.equal('class="classes anotherclass lastclass fg4"');
  ...
  it('should parse other attributes', function() {
    var string = '[onclick="doSomething();" href="#"]',
     ...
     .to.exist;
     expect(parsed.attributes)
     .to.equal('onclick="doSomething();" href="#"');
  });
  
  it('should parse everything combined', function() {
    var string =
    'section.class.anotherclass.BADNA@ME!*&6#ID#anotherID[data-attribute="attribute" onclick="doSomething();"]',
     parsed = parse(string);
     ...
     expect(parsed.attributes)
     .to.equal(
       'id="ID" class="class anotherclass BADNAME6" data-attribute="attribute" onclick="doSomething();"'
     );
  });
